...
from database import (
    get_book_by_isbn, get_patron_borrowed_books, get_patron_borrow_count,
    insert_book, insert_borrow_record, update_book_availability,
    update_borrow_record_return_date, get_all_books, get_book_by_id, get_db_connection
)
...
def return_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, str]:
    ...
    """
    # Verify the book was borrowed by the patron
    borrowed_list = get_patron_borrowed_books(patron_id)

    find_book = False
    for sub in borrowed_list:
        if sub['book_id'] == book_id:
            find_book = True
            break
    if not find_book:
        return False, "Book was not borrowed by patron."

    # Update available copies and record return date
    availability_success = update_book_availability(book_id, +1)
    if not availability_success:
        return False, "Database error occurred while updating book availability."
    ...
    # fees_owed = calculate_late_fee_for_book(patron_id, book_id)
...
def calculate_late_fee_for_book(patron_id: str, book_id: int) -> Dict:
    ...
    """
    # Get due date and return date
    conn = get_db_connection()
    patron_ = conn.execute('''
    ...
    if return_date > due_date:
        overdue = due_date - return_date
        if overdue <= 7:
            fees = 0.5*overdue
        else:
            past_seven = overdue - 7
            fees = (0.5*7) + past_seven
    ...
    return {
        ...
        'status': 'Late fee calculation implemented'
    }
